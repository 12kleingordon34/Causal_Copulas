...
def test_binary_non_chol_reshape_matrix():
	n_vars = 2
	rhos = [0.5]
	outcome_corr = cops.__reshape_matrix(n_vars, rhos, is_cholesky=False)
	target_outcome = jnp.array([[1,0.5], [0.5, 1]]) 
...
def test_three_class_non_chol_reshape_matrix():
	...
	rhos = [0.1, 0.2, 0.3]
	outcome_corr = cops.__reshape_matrix(n_vars, rhos, is_cholesky=False)
	target_outcome = jnp.array([[1,0.1, 0.2], [0.1, 1, 0.3], [0.2, 0.3, 1]]) 
	assert (outcome_corr == target_outcome).all()
